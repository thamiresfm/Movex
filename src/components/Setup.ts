...
export async function renderSetup(): Promise<void> {
      ...
      <div style="width:640px;max-width:100%;margin-top:auto;margin-bottom:auto;">
        ...
        <div id="step-1">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:36px;">
            <div class="mode-card selected" id="modeServer" onclick="selectMode('server')" style="background:var(--bg-3);border:1.5px solid var(--cyan);border-radius:14px;padding:28px 24px;cursor:pointer;transition:all .2s;background:linear-gradient(135deg,#0f1824,#0b0c10);box-shadow:0 0 24px rgba(0,212,255,.1);">
            ...
            <div class="mode-card" id="modeClient" onclick="selectMode('client')" style="background:var(--bg-3);border:1.5px solid var(--border);border-radius:14px;padding:28px 24px;cursor:pointer;transition:all .2s;">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;padding-top:4px;">
            ...
            <div style="display:flex;gap:12px;align-items:center;">
              ...
              <button class="btn-primary" onclick="goToStep2()">Próximo Passo</button>
            </div>
          </div>
        </div>
        ...
        <div id="step-2" style="display:none;">
          <div style="background:var(--bg-2);border:1px solid var(--border-c);border-radius:12px;padding:20px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;gap:16px;">
            ...
            <button class="btn btn-outline" id="btnCopy" onclick="copyPsk()">Copiar</button>
          </div>
          <p style="font-size:12px;color:var(--text-3);margin-bottom:28px;">💡 Use esta chave no outro computador ao instalar o Movex.</p>
          <div style="display:flex;gap:12px;justify-content:flex-end;">
            <button class="btn-ghost" onclick="showStep(1)">← Voltar</button>
            <button class="btn-primary" onclick="finishSetup()">Concluir Configuração ✓</button>
          </div>
        </div>
      </div>
  ...
  (window as any).selectMode = (mode: string) => {
    selectedMode = mode;
  ...
  (window as any).finishSetup = async () => {
