...
impl MultiMonitorLayout {
    ...
    pub fn to_normalized(&self, x: i32, y: i32) -> (f32, f32) {
        ...
        // `.max(1)` no denominador: igual a `normalize_point_against_display_rect`
        // e a boundary.rs, evita Inf/NaN com monitor degenerado de tamanho 0.
        (
            (x - bx) as f32 / bw.max(1) as f32,
            (y - by) as f32 / bh.max(1) as f32,
        )
    }
    ...
    pub fn to_absolute(&self, nx: f32, ny: f32) -> (i32, i32) {
        let (bx, by, bw, bh) = self.bounding_box();
        (bx + (nx * bw as f32) as i32, by + (ny * bh as f32) as i32)
    }
}
...
///
/// Limitações conhecidas (ambas aceites por agora — não há refactor planeado):
/// 1. Hotplug: o cache (`DESKTOP_BBOX_CACHE`, `OnceLock`) é preenchido na primeira
///    utilização e nunca invalidado. Adicionar/remover/reposicionar monitores em runtime
///    NÃO atualiza o bounding box — é preciso reiniciar a aplicação (ou reconectar) para
///    recalcular o layout.
/// 2. Scale per-monitor heterogéneo: o `scale_factor` usado a jusante assume o do monitor
///    primário. Configurações com monitores de escalas/DPI diferentes (ex. primário 2× +
///    secundário 1×) não são tratadas — o secundário herda a escala do primário. Refactor
///    de escala por-monitor foi deliberadamente evitado por risco de quebrar o multi-monitor
///    que já funciona.
#[cfg(any(target_os = "macos", target_os = "windows"))]
#[inline]
...
fn detect_monitors_macos() -> MultiMonitorLayout {
    ...
    let mut monitors: Vec<Monitor> = displays
        .iter()
        .map(|&id| {
    ...
    if monitors.is_empty() {
        ...
        let scale = if b.size.width > 0.0 {
            d.pixels_wide() as f32 / b.size.width as f32
        } else {
            1.0
        };
...
impl ScreenLayout {
    pub fn to_local_pixels(&self, x_norm: f32, y_norm: f32) -> (f32, f32) {
        (
            x_norm * self.local.width as f32,
            y_norm * self.local.height as f32,
        )
    }
    pub fn to_normalized(&self, x_px: f32, y_px: f32) -> (f32, f32) {
        (
            x_px / self.local.width as f32,
            y_px / self.local.height as f32,
        )
    }
}
